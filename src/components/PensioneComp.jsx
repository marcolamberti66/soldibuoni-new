...
// Fonte: https://www.covip.it/per-gli-operatori/fondi-pensione/costi-e-rendimenti-dei-fondi-pensione/elenco-dei-rendimenti
// ═══════════════════════════════════════════════════════════════════
...
export function PensioneComp({ color = '#14b8a6' }) {
  ...
  const filtered = useMemo(() => {
    ...
    // Filter out records with null value for selected horizon
    data = data.filter(d => d[horizon] !== null && d[horizon] !== undefined);
    
    // Sort by selected horizon descending (best first)
        ...
        <div key={`${d.f}-${d.c}-${i}`} className="fund-card" style={{ animationDelay: `${i * 0.03}s` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Rend. {activeHz?.label}</div>
              <RendPill value={d[horizon]} />
            </div>
          </div>

          {/* Barra rendimenti tutti gli orizzonti */}
