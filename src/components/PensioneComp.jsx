  ...
  return (
    <div className="pension-container">
      ...
      <div className="glass-panel">
        ...
        <div style={{ marginTop: 20, marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ flex: 1, minWidth: 180 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: '#475569', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Categoria</label>
            <select
              value={categoria}
              onChange={(e) => { setCategoria(e.target.value); setShowAll(false); }}
              style={{ width: '100%', padding: '10px 14px', fontSize: 14, fontWeight: 600, border: '1px solid #cbd5e1', borderRadius: 10, background: '#fff', color: '#0f172a', cursor: 'pointer', fontFamily: 'inherit' }}
            >
          ...
          <div style={{ flex: 1, minWidth: 180 }}>
            <label style={{ fontSize: 11, fontWeight: 800, color: '#475569', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Ordina per rendimento</label>
            <select
              value={horizon}
              onChange={(e) => { setHorizon(e.target.value); setShowAll(false); }}
              style={{ width: '100%', padding: '10px 14px', fontSize: 14, fontWeight: 600, border: '1px solid #cbd5e1', borderRadius: 10, background: '#fff', color: '#0f172a', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              {HORIZONS.map(hz => (
                <option key={hz.key} value={hz.key}>{hz.label} ({hz.sub})</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div key={`${d.f}-${d.c}-${i}`} className="fund-card" style={{ animationDelay: `${i * 0.03}s` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', flexShrink: 0 }}>
              <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', marginBottom: 2 }}>Rend. {activeHz?.label}</div>
